...
<body class="no-skin">
								...
								<c:when test="${r"${not empty varList}"}">
									...
									<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><a href="javascript:goSondict('${r"${var."}UUID${r"}"}')"><i class="ace-icon fa fa-share bigger-100"></i>&nbsp;${r"${var.NAME}"}</a></td>
										<#list fieldList as var>
											<#if var[3] != "否">
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${r"${var."}UUID${r"}"}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${r"${QX.del == 1 }"}">
													<a class="btn btn-xs btn-danger" onclick="del('${r"${var."}UUID${r"}"}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${r"${QX.edit == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="edit('${r"${var."}UUID${r"}"}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${r"${QX.del == 1 }"}">
															<li>
																<a style="cursor:pointer;" onclick="del('${r"${var."}UUID${r"}"}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<a class="btn btn-mini btn-success" onclick="add('${r"${"}UUID${r"}"}');">新增</a>
									</c:if>
									<c:if test="${r"${null != pd."}UUID${r" && pd."}UUID${r" != ''}"}">
	...
	<script type="text/javascript">
		...
		//去此ID下子级列表
		function goSondict(UUID){
			top.jzts();
			window.location.href="<%=basePath%>${objectNameLower}/list.do?UUID="+UUID;
		};
		...
		function add(UUID){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>${objectNameLower}/goAdd.do?UUID='+UUID;
			 diag.Width = 800;
			 diag.Height = 600;
			 ...
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					 parent.location.href="<%=basePath%>${objectNameLower}/listTree.do?UUID=${r"${"}UUID${r"}"}&dnowPage=${r"${page.currentPage}"}";
				}
				diag.close();
			 };
			 diag.show();
