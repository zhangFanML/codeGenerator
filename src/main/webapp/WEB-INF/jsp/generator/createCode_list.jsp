...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" style="width:200px;">表名</th>
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class="center">${fn:replace(var.TABLENAME, ',', '</br>')}</td>
											<%--<td class="center">
												${var.FHTYPE == "single"?"单/多表":""}
												${var.FHTYPE == "tree"?"树形":""}
	...
	<script type="text/javascript">
		...
		function productCode(CREATECODE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="${pd.flag == 'front' ? '前端':'后端'}代码生成";
			 diag.URL = '<%=basePath%>createCode/goProductCode?CREATECODE_ID='+CREATECODE_ID+"&flag="+"${pd.flag}";
			 diag.Width = 1300;
			 diag.Height = 700;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>createCode/delete?CREATECODE_ID="+Id+"&tm="+new Date().getTime();
