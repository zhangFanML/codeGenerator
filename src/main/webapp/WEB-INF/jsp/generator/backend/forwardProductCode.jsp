...
<body class="no-skin">
	<div class="main-container" id="main-container">
		<div class="main-content">
			...
			<div id="zhongxin">
			<form role="form" action="createCode/proCode?flag=${pd.flag}" name="Form" id="Form" method="post">
				...
				<input type="hidden" name="KEY" id="KEY" value="${pd.KEY}">
				<input type="hidden" name="FHTYPE" id="FHTYPE" value="backend"/>
				...
				<table id="table_report" class="table table-bordered" style="margin-top: 3px;">
					...
					<tr>
						...
						<th class="center" style="width:180px;">表名</th>
						<th class="center" style="width:180px;">字段名称</th>
						<th class="center" style="width:180px;">注释</th>
						<th class="center" style="width:100px;">字段类型</th>
						<th class="center" style="width:180px;">字典</th>
						<th class="center" style="width:60px;">长度</th>
						<th class="center" style="width:60px;">小数位</th>
						<th class="center" style="width:79px;display: none">表单录入</th>
						<th class="center" style="width:79px;display: none">是否必输</th>
						<th class="center" style="width:79px;display: none">列表显示</th>
						<th class="center" style="width:79px;">是否查询</th>
						<th class="center" style="width:79px;">是否模糊查询</th>
						<th class="center" style="width:70px;">字段默认值</th>
				...
				<div id="zhongxin3" class="center" style="position:fixed;z-index: 80001;top: 0%;left: 0%;display:none;width: 100%;height: 100%;padding: auto;background-color: rgba(212,216,184,0.2);">
					<div style="position:fixed;top:40%;left: 45%;">
						<img src="static/images/jiazai.gif"/>
						<h4 class="lighter block green">
...
<script type="text/javascript">
	...
	function selectTableRow(){
		...
		if(tabletop != null && tabletop != "" && tabletop != ''){
			...
			$.ajax({
				type:"post",
				url:"<%=basePath%>createCode/getTableRow?flag=backend&tabletop="+tabletop + "${info != null ? info : ""}",
				dataType:"text",
	...
	function appendRow(value){
		...
		$("#fields").append(
				...
				'<td class="center" style="padding: 8px auto">' +
					'<label class="pull-right inline">' +
					'<small class="muted smaller-90"><font style="vertical-align: inherit;"></font></small>' +
	...
	//列 的显隐
	function tackTabletops(Obj){
		var tabletop = $("#tabletop").val();
		if(tabletop != null && tabletop.toString().split(",").length > 1){
			$("#table_report").find('th:eq(7)').hide();
			$("#table_report tr").find('td:eq(7)').hide();
		}else{
			$("#table_report").find('th:eq(7)').show();
			$("#table_report tr").find('td:eq(7)').show();
		}
	}
	...
	function saveL(){
		...
		var selval = $("#taL select");
		var switchh = false;
		for (var i = 0; i < selval.length; i++) {
			if(selval.eq(i).val() == "" || selval.eq(i).val() == null){
				selval.eq(i).tips({
					side:3,
					msg:'请选择关联配置',
					bg:'#AE81FF',
