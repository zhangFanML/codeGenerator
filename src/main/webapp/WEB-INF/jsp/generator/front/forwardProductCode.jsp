	...
	<div class="main-container" id="main-container">
        ...
        <div id="zhongxin">
			<form action="createCode/proCode?flag=${pd.flag}" name="Form" id="Form" method="post">
                ...
                <input type="hidden" name="FHTYPE" id="FHTYPE" value="front"/>
				    ...
				    <table id="table_report" class="table table-bordered" style="margin-top: 3px;">
						...
						<thead>
							<tr>
								...
								<th class="center"style="width:150px;display: none">表名</th>
								<th class="center"style="width:250px;">属性名</th>
								<th class="center">属性说明</th>
								<th class="center" style="width:100px;">输入类型</th>
								<th class="center" style="width:100px;">字典/Tree</th>
								<th class="center" style="width:60px;">长度</th>
								<th class="center" style="width:60px;display: none">小数位</th>
								...
								<th class="center" style="width:79px;display: none">是否模糊</th>
								<th class="center" style="width:70px;">属性默认值</th>
...
<script type="text/javascript">
	...
	function selectTableRow(){
		...
		if(tabletop != null && tabletop != "" && tabletop != ''){
			...
			$.ajax({
				type:"post",
				url:"<%=basePath%>createCode/getTableRow?flag=front&tabletop="+tabletop + "${info != null ? info : ""}",
				dataType:"text",
				...
				success:function(data){
					if(data != null && data != "" && data != ''){
						...
						if(((fieldarray[fieldarray.length -1]).indexOf("@")) == -1){
							$("#KEY").val(fieldarray[fieldarray.length -1]);
	...
	function appendRow(value){
		...
		$("#fields").append(
				...
				'<td class="center" style="display: none;" onmouseenter="changeToInput(this)" onmouseleave="changeToText(this)" >'+fieldarray[10]+'</td>'+	//表名
				...
				'<td class="center" onmouseenter="changeToInput(this,\'select\')" onmouseleave="changeToText(this)" >'+fieldarray[1]+'</td>'+	    //输入类型
				...
				'<td class="center" style="display: none;" onmouseenter="changeToInput(this,\'Double\')" onmouseleave="changeToText(this)" >'+fieldarray[6]+'</td>'+	    //小数点
				'<td class="center">' +
	...
	function addTableRow(){
		...
		$("#fields").append(
				'<tr onclick="changeTrOrder(this)" draggable="true">'+
					'<td class="center" >'+Number(index+1)+'</td>'+
					'<td class="center" style="display: none;" onmouseenter="changeToInput(this)" onmouseleave="changeToText(this)" ></td>'+	//表名
					...
					'<td class="center" onmouseenter="changeToInput(this,\'select\')" onmouseleave="changeToText(this)" ></td>'+	    //输入类型
					...
					'<td class="center" style="display: none;" onmouseenter="changeToInput(this,\'Double\')" onmouseleave="changeToText(this)" >0</td>'+	    //小数点
					'<td>' +
						'<label class="pull-right inline">' +
						'<small class="muted smaller-90"><font style="vertical-align: inherit;"></font></small>' +
		...
		var isRequired = (tds.eq(9).find("input").val()); //是否必输
		var isShow = (tds.eq(10).find("input").val());      //是否列表显示
		var isQuery = (tds.eq(11).find("input").val());    //是否查询
		var isVague = (tds.eq(12).find("input").val());      //是否模糊查询
		//          属性名0        字段类型1     属性说明2    是否表单录入3        默认值4             长度5           小数位6            是否必输7            字典8      是否列表显示9       表名10         是否查询11      是否模糊查询12
	...
	function getFields(Obj,change){
		...
		var isQuery = (tds.eq(11).find("input").val());     //是否查询
		var isVague = (tds.eq(12).find("input").val());     //是否模糊查询
		//           属性名0        输入类型1     属性说明2    是否表单录入3       默认值4             长度5           小数位6            是否必输7            字典8      是否列表显示9       表名10         是否查询11      是否模糊查询12
		var fields = dname + '|@|' + dtype + '|@|' + dbz + '|@|' +isQian + '|@|' + ddefault + '|@|' + flength + '|@|' + decimal + '|@|' + isRequired + '|@|' + dict + '|@|' + isShow + '|@|' + table + '|@|' + isQuery + '|@|' + isVague ;
	...
	function changeToInput(Obj,flag){
		...
		if(flag == "select"){
			html = '<select  data-placeholder="请选择数据类型"  style="inputStyle" onchange="getFields(this,'+'\'change\''+')" value="' + value + '">\n' +
					...
					'\t\t\t\t\t\t\t\t<option value="Tree" ' + (value == "Tree" ? "selected" : "") +'>Tree</option>\n' +
					'\t\t\t\t\t\t\t\t<option value="Phone" '+ (value == "Phone" ? "selected" : "") +'>Phone</option>\n' +
					'\t\t\t\t\t\t\t\t<option value="Textarea" '+ (value == "Textarea" ? "selected" : "") +'>Textarea</option>\n' +
					...
					'\t\t\t\t\t\t\t\t<option value="Switch" ' + (value == "Switch" ? "selected" : "") +'>Switch</option>\n' +
					'\t\t\t\t\t\t\t</select>';
		}else if(flag == "Dictionary"){
			if(obj.prev().html() != "Dictionary" && obj.prev().html() != "Tree"){
				return;
			}
		}else if(flag == "Double"){
